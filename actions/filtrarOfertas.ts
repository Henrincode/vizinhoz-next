...
// Por hora filtra somente por tipo
export async function filtrarOfertas(filtro: any) {
    if(!filtro) return ofertas
    const lista = ofertas.filter(p => p.tipo === filtro || filtro === 'tudo')
    return lista
}